...
Converted to a Network Flow graph with bandwidths assigned to each edge. Download the dataset from the above link and unzip into the `datasets/caida/` directory. Use the `/srs/preprocess/caida_2_flow.py` script to convert the dataset. The bandwidths are generated based on the degree of the nodes, with some random noise added to simulate real-world variability. The node ids used in the final graph are dense ids starting from 0, which are mapped from the original AS numbers. 
